...
public class BibekGenerator
{
   ...
   // Dictionary<string, UnityEngine.Tilemaps.Tile> tilePair = new Dictionary<string, UnityEngine.Tilemaps.Tile>();
    ...
    public BibekGenerator(int xDimension, int yDimension)
    {
    ...
    private void SetupStandardPallette()
    {
        ...
        BibekStandardCell topSc = new BibekStandardCell(t);
        BibekStandardCell bottomSc = new BibekStandardCell(b);
        BibekStandardCell verticialSc = new BibekStandardCell(vh);
        BibekStandardCell horizontalSc = new BibekStandardCell(hh);
        BibekStandardCell topLeftSc = new BibekStandardCell(tl);
        BibekStandardCell topRightSc = new BibekStandardCell(tr);
        BibekStandardCell bottomLeftSc = new BibekStandardCell(bl);
        BibekStandardCell bottomRightSc = new BibekStandardCell(br);
        BibekStandardCell leftSc = new BibekStandardCell(l);
        BibekStandardCell rightSc = new BibekStandardCell(r);
    ...
    private void SetupInitialGrid()
    {
        ...
        for (int i = 0; i < world_width; i++)
        {
            for (int j = 0; j < world_heigth; j++)
            {
                ...
                BibekCell newCell = new BibekCell(i, j);
                grid[i, j] = newCell;
            }
        }
    ...
    private BibekCell FindCellLowestEntropy(BibekCell[,] grid)
    {
        int lowestEntropy = 999999; // arbitrarially large value for start Entropy
        List<BibekCell> lowestCells = new List<BibekCell>();
        BibekCell chosenLowestCell = null;

        for (int j = 0; j < world_heigth; j++)
        {
            for (int i = 0; i < world_width; i++)
            {
                BibekCell tmpCell = grid[i, j];
    ...
    private List<string> CombineValidNeighbors(BibekCell curRefCell, string direction)
    {
        ...
        foreach (string curOpt in curRefCell.options)
        {
            ...
            if (direction == "right") combinedAllowedNeigbhors.AddRange(tmpBibekStandardCell.rightNeighbors);
            if (direction == "down") combinedAllowedNeigbhors.AddRange(tmpBibekStandardCell.downNeighbors);
            if (direction == "left") combinedAllowedNeigbhors.AddRange(tmpBibekStandardCell.leftNeighbors);
        }
    ...
    public bool RemoveInvalidOptionsFromNeighbor(List<string> allowedNeighbors, int neighborX, int neighborY)
    {
        ...
        {
            BibekCell curNeighborCell = grid[neighborX, neighborY];
    ...
    public void DumpStackNames(Stack<BibekCell> stack)
    {
        ...
        Debug.Log("-----------------");
        foreach (BibekCell cell in stack)
        {
    ...
    //////////////////////////////////////////////////////////////////////////////////////////
    private string CollapseCell(ref BibekCell cell)
    {
        string collapsedVal = "";
    ...
    private void Propagate_Expanded(BibekCell collapsedCell)
    {
        ...
        while (cellsToAddress.Count > 0)
        {
            ...
            // BibekStandardCell rules for the currect collapsed cell
            ...
            {
                BibekCell neighborCell = grid[curCellX, curCellY - 1];
            ...
            {
                BibekCell neighborCell = grid[curCellX + 1, curCellY];
    ...
    // 
    //////////////////////////////////////////////////////////////////////////////////////////
    private void Propagate_Condensed(BibekCell collapsedCell)
    {
        // We will use a stack to hold the cells that need to be propagated
        // Add the current cell to the stack to start off
        Stack<BibekCell> cellsToAddress = new Stack<BibekCell>();
    ...
    private void ProcessNeighborRestrictions(Stack<BibekCell> cellsToAddress, BibekCell curRefCell, int tmpX, int tmpY, string direction)
    {
        if (CanCellBePropagated(tmpX, tmpY))
        {
            BibekCell neighborCell = grid[tmpX, tmpY];
    ...
    public void PerformWFC()
    {
        ...
        while (moreToProcess)
        {
            ...
            BibekCell cellToCollapse = FindCellLowestEntropy(grid);
